...
try {
    ...
    if (-not $InputPath) {
        $InputPath = Join-Path $Root "sample_data\live_case_input.json"
    }
    if (-not $OutputPath) {
        $OutputPath = Join-Path $Root "data\live_ops\live_case_result.json"
    }
    if (-not $MarkdownPath) {
        $MarkdownPath = Join-Path $Root "data\live_ops\live_case_report.md"
    }

    if (-not (Test-Path $InputPath)) {
        Write-Error "Missing live case input: $InputPath"
        exit 2
    }
    $runtimePath = Join-Path $Root "app\live_ops\live_case_loop.py"
    if (-not (Test-Path $runtimePath)) {
        Write-Error "Missing live case runtime: $runtimePath"
        exit 2
    }

    Write-Host ""
    Write-Host "TDK LIVE CASE LOOP :: ISOLATED DEMO" -ForegroundColor Cyan
    ...
    if ($code -ne 0) {
        ...
        Write-Error "LIVE CASE LOOP demo failed with exit code $code"
        exit 1
    }
    $output | ForEach-Object { Write-Host $_ }

    Write-Host ""
    Write-Host "live-case-demo-ok" -ForegroundColor Green
    exit 0
} catch {
    Write-Error ("LIVE CASE LOOP controlled failure: {0}" -f $_.Exception.Message)
    exit 1
}
